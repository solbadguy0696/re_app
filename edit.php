...
<body>
  ...
  <form action="store.php" method="post">  <!--store.phpに遷移-->
    ...
    <input type="hidden" name="id" value="<?php echo h($_GET['id']) ?>">  <!--$_GET['id']でURLクエリのデータを取得しそれをh関数に渡す。戻り値を出力する-->
    <input type="text" name="todo" value="<?php echo h($data) ?>">  <!--$dataを引数としてそれをh関数に渡す。戻り値を出力する-->
